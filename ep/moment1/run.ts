import { Planet, Resource, Spaceship, Event } from "./exports";
...
function promptCollectResources() {
 const rl = readline.createInterface({
 input: process.stdin,
 output: process.stdout
 });

 rl.question(`Do you want to collect ${planet1.commonResource}? (Y/N): `, (answer) => {
 if (answer.toUpperCase() === 'Y') {
  // Call the collectResources function here
  fn.collectResources<number>(planet1, 10);
 } else if (answer.toUpperCase() === 'N') {
  console.log('Resource collection skipped.');
 } else {
